...
- **voice/** - speech dictation. `voiceStore.ts` is the single source of truth: a Zustand store holding `status` (`idle`/`arming`/`recording`/`transcribing`) plus a `hold` state driven by the pure reducer in `holdIntent.ts`. Push-to-talk is driven by **intent, not observed status** - the reducer decides start/stop/cancel from key events, so a slow transcription can never be read as a released key. A tap latches (hands-free until pressed again), a real hold stops on release, and `holdTimeoutMs` arms a safety timer per phase so a stuck key or a dropped keyup cannot record forever. Blur and tab-hide cancel; Escape cancels from anywhere. `usePushToTalk.ts` owns the key plumbing, including the macOS Fn key, whose native monitor (`voice_set_fn_monitor`, `src-tauri/src/modules/voice.rs`) is installed only while the Fn binding is actually enabled, so the OS hook costs nothing otherwise. `useVoiceController.ts` bridges the store to `ai/hooks/useWhisperRecording` and applies the result to whichever target `App.tsx` resolves - the AI composer, the focused editor, or the active terminal leaf. Terminal dictation goes through `pasteIntoSession`, not `writeToSession`, so text lands in the Ghostty block prompt input the user is looking at instead of behind it as raw PTY bytes. `cleanup.ts` optionally runs the raw transcript through a model to repair punctuation and drop disfluencies, guarded by `keepsFullTranscript`: if the model returns less than `CLEANUP_MIN_RETENTION` of the original it is discarded and the raw transcript wins, so cleanup can never silently eat a long dictation. Long transcripts skip the model call entirely (`shouldSkipCleanup`). `VoiceHud.tsx` is the overlay with the live status and a cancel control. The recorder side is deferred: only the store and the key plumbing are eager, and `VoiceRuntimeLazy.tsx` mounts `VoiceRuntime` (controller + cleanup + HUD, a ~3 kB chunk) the first time an intent would actually start recording. Because the keys stay eager the first press is never dropped - the store records it as a `pending` effect and `bindImpl` replays it once the chunk lands, so a press still held when the runtime binds starts recording and one already released does not. `setStatus("idle")` deliberately does not reset `hold` while an effect is pending, otherwise the controller's own mount would wipe the press that triggered the load. Note the whisper recorder itself stays eager regardless, because `ai/lib/composer.tsx` mounts its own `useWhisperRecording` for the composer mic.
...
- **Voice input**: `hooks/useWhisperRecording.ts` is the recorder and transcription call (provider and model from `sttProvider` / `sttOptions`). It is a dumb capture layer: lifecycle, intent, targeting and transcript cleanup all live in `modules/voice/`. Reachable from the composer, the shortcut registry (`voice.toggle`, `voice.cancel`) and the command palette.
