...
- **terminal/** — `TerminalStack` keeps one mounted xterm per tab via `useTerminalSession` + `pty-bridge`. `osc-handlers.ts` parses OSC 7 (with Windows drive-letter normalization: `/C:/Users/foo` → `C:/Users/foo`) and OSC 133 **A** (prompt line) + **B** (PS1 start) via `registerShellIntegrationMarkers`. Themes in `themes.ts`. Optional **line autocomplete** (`lib/autocomplete/`, Settings → General): grey inline ghost + dropdown from an in-memory history ring plus a static command catalog (`staticCommands.ts`: Unix-wide list merged at load with **macOS-only** or **Linux-only** extras via `@/lib/platform`); **Tab** accepts; **Esc** closes the multi-match dropdown first, a second **Esc** snoozes autocomplete for the rest of that prompt (cleared on **Enter** or the next shell prompt); **Esc** once when only a ghost shows snoozes immediately; only active on the prompt input line (OSC **133 B** marker when present, else **A**) and when the setting is on. Prefix is tracked from user `onData` (not PTY echo); heavy PTY output throttles recomputation. History is per terminal session, not persisted. Debug: `localStorage.setItem('terax-debug-terminal-autocomplete','1')` then reload — console shows `[terax:terminal-ac]` lines with `cursorAbs`, OSC marker lines, and `prefix` on each recompute.
