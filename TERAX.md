...
- **explorer/** - file tree with Material/Catppuccin icons (`iconResolver.ts`), fuzzy search, keyboard nav, inline rename, context actions. Backslash-aware `basename`. It consumes the usable active Space root; opening an external file never retargets that root.
- **preview/** - auto-detected dev-server preview tab (status-bar pill suggests opening when a localhost URL is detected).
- **tabs/** - `useTabs` is the source of truth for tab list + active id. New terminals start at their owning Space root; `basename` splits on both `/` and `\`.
...
- **statusbar/** - bottom bar, Space-root breadcrumb (handles Unix paths, Windows drive letters, and home `~` segments via `pathUtils.segmentsFromCwd`), AI tools indicator.
...
- **source-control/** - git status / stage / commit panel and diff workflow, rooted only at the usable active Space root. It has no independent follow state or fixed repository target.
...
- **workspace/** - workspace environment utilities and authorization (Local + WSL distros).
...
### Space-owned workspace context

`SpaceMeta.root` and `SpaceMeta.env` are the sole workspace authority. An environment is fixed when its Space is created. Terminal and pane cwd, including OSC 7 updates, are runtime-only state and never change a Space root or environment.

A new `Space N` is created at the selected environment's Home. The bottom bar shows the full Space-root breadcrumb and a trailing `...` child-directory menu scoped to that environment. There is no system directory picker.

Explorer and Source Control consume only the usable active Space root. Opening external files does not mutate the root, and Source Control has no fixed repository target. Changing a root updates Explorer, Source Control, and future terminals only; running terminals keep their cwd.

At boot, an invalid root recovers to Home in the same environment. A failed WSL Home lookup never falls back to Local. If environment adoption fails, a rootless fallback blocks persistence. Authorization for restored and live terminal cwd uses the terminal's owning Space environment.
