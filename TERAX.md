...
- `speech::*` (`stt_native_status`, `stt_native_install`, `stt_native_transcribe`): managed local transcription. A signed on-demand sidecar uses Speech Swift on Apple silicon macOS 15+ and Speech Core on x86-64 Linux/Windows. Models are revision-pinned, downloaded with bounded streaming, and verified by SHA-256 when Hugging Face publishes an LFS digest. Audio crosses raw IPC as finite mono Float32 PCM through a versioned binary protocol. The sidecar is fixed by platform, runs in its own process group or Windows Job Object, keeps only one model resident, and stops after five idle minutes.
...
- **Voice input**: streamed recording pipeline toggled from the composer. Cloud STT uses OpenAI or Groq. Whisper.cpp remains available through its loopback `/inference` server with redirects disabled. Terax Native uses Nemotron by default and Parakeet as its low-memory profile. The webview decodes and downsamples recordings to 16 kHz mono Float32 PCM; Rust validates the raw IPC body and forwards it to a persistent private sidecar without HTTP, temporary audio files, or arbitrary executable input. Runtime archives are release assets signed with the updater key. Models and runtimes are absent until installed, and only one model is resident at a time.
