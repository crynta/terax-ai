...
| Done locally | Pass pnpm and Rust verification gates. | Latest post-merge local checks include `pnpm install --frozen-lockfile --offline`, `pnpm format:check`, `pnpm lint` (185 warnings only), `pnpm exec tsc --noEmit`, `pnpm test` (198 files, 1170 tests), `pnpm test:coverage` (198 files, 1170 tests), `pnpm check:pi-boundary`, `pnpm build`, `pnpm check:bundle-size`, and `pnpm tauri build --bundles app --no-sign --ci`. Earlier post-merge Rust validation passed `cargo fmt -- --check`, `cargo check`, `cargo test`, `cargo check --features workflow`, and `cargo check --features openclicky`. | CI must independently run on the PR. |

## Latest local automated verification

Post-merge/current-head checks run from `b73b79aa1501d36c888d609affd4b9be644b8c58`:
...
1. Approve/re-run GitHub Actions for PR #964 and confirm the CI matrix plus Linux e2e are green.
