...
- Visible checks: at `13c6adc16`, `gh pr checks` showed CodeRabbit `pass` with "Review skipped: 867 files exceed the limit of 150" and no GitHub Actions runs. This branch now adds `workflow_dispatch` to CI, but the current base/default workflow will not expose manual triggering until maintainers resolve conflicts and accept or merge that workflow change.
...
| Blocked | Confirm CI/e2e green. | `gh pr checks 964 --repo crynta/terax-ai` reports only CodeRabbit states, not GitHub Actions; `gh pr view` reports `mergeStateStatus=DIRTY`; earlier `gh run list --repo crynta/terax-ai --workflow CI --branch pi-sidebar --limit 20` returned no Actions runs; fork workflow/run lists return no workflows or runs. Branch CI config now includes `workflow_dispatch`, and `ruby -e 'require "yaml"; YAML.load_file(".github/workflows/ci.yml")'` parses it. | GitHub Actions and Linux e2e are not available for this dirty fork PR; maintainer must resolve conflicts and let base PR CI run or accept the workflow dispatch change before manual triggering is possible. |
