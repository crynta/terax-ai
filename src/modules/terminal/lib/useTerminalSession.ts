...
function ensureSession(leafId: number, initialCwd?: string): Session {
  ...
  session.detectSsh = createSshCommandDetector((detected) =>
    session.callbacks.onDetectedSsh?.(detected),
  );
  ...
  useEffect(() => {
    ...
    s.ready.then(() => {
      ...
      attachSession(leafId, container.current, {
        ...
        onDetectedSsh: (d) => cbRef.current.onDetectedSsh?.(d),
